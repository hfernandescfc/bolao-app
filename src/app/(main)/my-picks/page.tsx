...
export default async function MyPicksPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-bold text-gray-900">Meus Palpites</h1>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">Palpites de Partidas ({matchPicks.length}/72)</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          <div className="divide-y divide-gray-50 max-h-96 overflow-y-auto">
            {matchPicks.length === 0 && (
              <p className="text-center text-gray-400 text-sm py-6">Nenhum palpite ainda.</p>
            )}
            {matchPicks.map((p) => {
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">Palpites de Grupos ({groupPicks.length}/12)</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          <div className="divide-y divide-gray-50">
            ...
            {groupPicks.map((p) => {
              ...
              return (
                <div key={p.id} className="flex items-center justify-between px-4 py-2.5">
                  <span className="text-xs text-gray-600">Grupo {p.group_id}</span>
